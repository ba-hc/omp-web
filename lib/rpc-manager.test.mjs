...
test("RPC session startup resolves and passes the SDK-native enabled model scope", async () => {
  ...
  assert.match(startupSource, /modelWithWorkspaceContext\(initial\.model/);
...
test("RPC session startup persists explicit preferences without replaying setters", async () => {
  ...
  assert.doesNotMatch(startupSource, /modelDefaultChanged/);
});
...
test("RPC running state includes SDK-owned async work", () => {
  const inner = makeInner({ hasPendingAsyncWork: () => true });
...
test("AgentSessionLike exposes the SDK async-work contract", async () => {
  const source = await readFile(new URL("./omp-types.ts", import.meta.url), "utf8");

  assert.match(source, /getAsyncJobSnapshot\?\(/);
  assert.match(source, /hasPendingAsyncWork\?\(\): boolean/);
});
